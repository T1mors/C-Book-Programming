...
int main(int argc, char *argv[])
{
  ...
  double sum;                         /* for average of estimate of height */
  int t;                                                   /* loop counter */
  int *a;                                         /* array for permutation */
  int size;                                               /* size of array */
  double frac_ex;                                 /* fraction of exchanges */
  int num_ex;                                       /* number of exchanges */
  int run, num_runs;                      /* averaging over many sequences */

  if(argc != 3)
  {
    fprintf(stderr, "USAGE: %s <n> <frac_ex>\n", argv[0]);
    exit(1);
  }

  size = atoi(argv[1]);                    /* read input from command line */
  sscanf(argv[2], "%lf", &frac_ex);

  num_ex = (int) floor(frac_ex*size);                    /* allocate array */
  a = (int *) malloc(size*sizeof(int));

  sum = 0;
  num_runs = 1000;
  for(run=0; run<num_runs; run++)     /* average over many independen runs */
  {
    for(t=0; t<size; t++)                           /* setup ordered array */
      a[t] = t;
    tree_exchange( a, size, num_ex);
    ...
    sum += tree_height(tree);                     /* statistics of heights */
  ...
  printf("%d %f %f\n", size, (double) num_ex/ size, sum/num_runs);
