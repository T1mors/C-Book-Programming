...
void queens(int c, int N, int *pos, int *row,
            int *diag_up, int *diag_down, int *counts)
{
  ...
  for (r = N - 1; r >= 0; r--) /* place queen in all rows of column c */
  {
    ...
    {
      // printf("True: Column %d Row %d\n", c, r);
      row[r] = 1;
      diag_up[c - r + (N - 1)] = 1;
      diag_down[c + r] = 1;
      pos[c] = r;
      queens(c - 1, N, pos, row, diag_up, diag_down, counts);
      row[r] = 0;
      diag_up[c - r + (N - 1)] = 0;
      diag_down[c + r] = 0;
    }
...
int main(int argc, char *argv[])
{
  ...
  queens(N - 1, N, pos, row, diag_up, diag_down, &counts); /* do work */
  printf("counts: %d\n", counts);
